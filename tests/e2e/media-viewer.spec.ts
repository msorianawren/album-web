...
test("public viewer deep-links media, navigates, and closes back to the album", async ({ page }) => {
  ...
  await page.evaluate(() => {
    document.documentElement.dataset.chimeImpulseCount = "0";
    window.addEventListener("oriana-chime-impulse", () => {
      document.documentElement.dataset.chimeImpulseCount = String(Number(document.documentElement.dataset.chimeImpulseCount ?? "0") + 1);
    }, { once: true });
  });
  await page.getByRole("button", { name: "Play the wind chime" }).first().click();
  await expect.poll(() => page.locator("html").getAttribute("data-chime-impulse-count")).toBe("1");
