...
test.describe("Facebook professional-profile feed", () => {
  ...
  test("loads Facebook only after play, traps focus, and stays within a mobile viewport", async ({ browser }) => {
    ...
    try {
      ...
      await expect(dialog.getByRole("link", { name: "View on Facebook" })).toBeVisible();
      await page.keyboard.press("Shift+Tab");
      await expect(dialog.getByRole("link", { name: "View on Facebook" })).toBeFocused();
