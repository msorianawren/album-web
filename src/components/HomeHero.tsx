        ...
        <div className="page-shell-1440 grid min-w-0 items-center gap-16 lg:grid-cols-[1.2fr_0.8fr]">
          <ScrollReveal className="relative z-10 order-2 flex min-w-0 flex-col justify-center lg:order-1">
            ...
            <div className="mt-12 flex flex-wrap items-center gap-8 border-t border-border/50 pt-8">
              ...
              <a href={secondaryHref} className="flex items-center gap-3 text-[0.7rem] font-semibold uppercase tracking-[0.2em] text-text-secondary transition-colors duration-300 hover:text-text-primary">
          ...
          <ScrollReveal delay={0.2} className="relative z-0 order-1 lg:order-2">
            <DepthSurface glare className="aspect-[3/4] w-full">
              <div className="relative h-full w-full overflow-hidden rounded-[2rem] bg-surface-secondary/40">
  ...
  if (preset === "split") {
    return (
      <section className="relative isolate flex min-h-[90vh] flex-col justify-center overflow-hidden">
        <div className="absolute inset-0 grid h-full min-w-0 lg:grid-cols-2">
          ...
          <div className="relative z-10 flex flex-col justify-center bg-background/55 px-6 pb-20 pt-32 sm:px-16 lg:bg-transparent lg:px-24 lg:py-0 xl:px-32">
            <ScrollReveal className="max-w-2xl">
              <p className="mb-6 text-[0.65rem] font-medium uppercase tracking-[0.2em] text-text-secondary">{eyebrow}</p>
              <h1 className="mb-6 text-[2.8rem] font-medium leading-[1.1] text-text-primary sm:text-5xl lg:text-6xl">{headline}</h1>
              <p className="mb-10 text-lg leading-relaxed text-text-secondary/90">{subheadline}</p>
              <div className="flex items-center gap-6">
                <a href={primaryHref} className="inline-flex h-11 items-center justify-center bg-text-primary px-6 text-[0.75rem] font-semibold uppercase tracking-[0.1em] text-background transition-transform hover:-translate-y-0.5">{primaryCtaLabel}</a>
                <a href={secondaryHref} className="text-[0.75rem] font-semibold uppercase tracking-[0.1em] text-text-secondary transition-colors hover:text-text-primary">{secondaryCtaLabel}</a>
              </div>
            </ScrollReveal>
          </div>
        </div>
      ...
      <div className="page-shell-1440 grid min-w-0 gap-8 pb-8 pt-6 sm:pb-16 sm:pt-16 lg:min-h-[760px] lg:grid-cols-[0.86fr_1.14fr]">
          ...
          <div className="mt-8 grid max-w-xl grid-cols-1 overflow-hidden rounded-[1.4rem] border border-border bg-surface/35 shadow-xl shadow-text-primary/5 sm:mt-10 sm:grid-cols-3">
            {stats.map((stat) => (
              <div key={`${stat.label}-${stat.value}`} className="min-w-0 border-b border-border p-4 transition-colors hover:bg-surface/50 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0">
                ...
                <p className="mt-1 break-words text-[0.68rem] font-semibold uppercase text-text-secondary">{stat.label}</p>
              </div>
            ))}
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal delay={0.2} className="relative z-0 hidden animate-editorial-in-slow sm:min-h-[500px] lg:block lg:min-h-[680px]">
          <DepthSurface glare strength="gentle" className="relative h-[28rem] sm:absolute sm:inset-x-10 sm:top-0 sm:h-[74%] lg:inset-x-0">
            <div data-nature-surface="hero-frame" className="relative h-full overflow-hidden rounded-[1.6rem] border border-border bg-surface/30 shadow-2xl shadow-text-primary/10 sm:rounded-[2.2rem]">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={landing.hero_image_url} alt="" className="hero-image-pan h-full w-full object-cover" />
          ...
          <DepthSurface strength="subtle" className="relative w-full sm:absolute sm:bottom-0 sm:left-0 sm:w-[46%]">
            <div data-nature-surface="hero-frame" className="relative w-full overflow-hidden rounded-[1.4rem] border border-border bg-surface/38 p-2 shadow-2xl shadow-text-primary/12 sm:rounded-[1.6rem]">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={landing.portrait_image_url} alt="" className="aspect-[4/5] w-full rounded-[1.15rem] object-cover transition duration-700 hover:scale-[1.03]" />
            </div>
          </DepthSurface>
          ...
          <div className="relative w-full rounded-[1.4rem] border border-border bg-surface/42 p-3 shadow-2xl shadow-text-primary/10 sm:absolute sm:bottom-8 sm:right-0 sm:w-[52%] sm:rounded-[1.6rem]">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-[0.8fr_1fr]">
              <div className="overflow-hidden rounded-[1.1rem]">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={landing.gallery_image_url} alt="" className="aspect-[4/5] h-full w-full rounded-[1.05rem] object-cover transition duration-700 hover:scale-[1.04]" />
              </div>
              <div className="flex min-w-0 flex-col justify-between p-2">
                <p className="text-xs font-semibold uppercase text-text-secondary">{dict?.landing?.studio_note || "Studio note"}</p>
                <p className="mt-2 text-sm leading-6 text-text-primary">{featureBody}</p>
