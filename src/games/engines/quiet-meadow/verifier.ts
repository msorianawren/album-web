...
export function verifyQuietMeadow(
  ...
  if (typeof config.width !== "number" || typeof config.height !== "number" || typeof config.totalMines !== "number") {
    return { valid: false, reason: "Invalid difficulty config", versionId: version.id, replayDigest: "0", score: 0, durationTicks: 0 };
  }
  ...
  for (const action of trace.actions) {
    if (action.tick <= lastTick) {
      return { valid: false, reason: "Invalid action sequence", versionId: version.id, replayDigest: "0", score: 0, durationTicks: 0 };
    }
    lastTick = action.tick;

    if (state.status === "won" || state.status === "lost") {
      return { valid: false, reason: "Action after terminal state", versionId: version.id, replayDigest: "0", score: 0, durationTicks: 0 };
    }

    const payload = action.payload as QuietMeadowActionPayload;
    if (!payload || typeof payload.x !== "number" || typeof payload.y !== "number") {
      return { valid: false, reason: "Invalid action payload", versionId: version.id, replayDigest: "0", score: 0, durationTicks: 0 };
    }

    if (payload.x < 0 || payload.x >= config.width || payload.y < 0 || payload.y >= config.height) {
      return { valid: false, reason: "Coordinate out of bounds", versionId: version.id, replayDigest: "0", score: 0, durationTicks: 0 };
    }
    ...
    if (action.type === "reveal") {
      ...
      return { valid: false, reason: "Unknown action type", versionId: version.id, replayDigest: "0", score: 0, durationTicks: 0 };
    }

    if (!success) {
      return { valid: false, reason: "Invalid action ignored by engine", versionId: version.id, replayDigest: "0", score: 0, durationTicks: 0 };
    }
  }

  // Scoring: 
  // Base score based on revealed cells. 1 point per safe cell.
