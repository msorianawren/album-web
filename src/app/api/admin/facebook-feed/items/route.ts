...
export async function POST(request: NextRequest) {
  ...
  } catch (error) { return error instanceof Error && error.message.startsWith("Use ") || error instanceof Error && error.message.startsWith("Enter ") ? apiError("INVALID_INPUT", error.message, 400) : toServerError(error, request, "facebook-feed.create"); }
}
