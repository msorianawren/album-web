...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  } catch (error) { return error instanceof FacebookFeedInputError || error instanceof Error && (error.message.startsWith("Use ") || error.message.startsWith("Enter ") || error.message.startsWith("Published ")) ? apiError("INVALID_INPUT", error.message, 400) : toServerError(error, request, "facebook-feed.update"); }
}
