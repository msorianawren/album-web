...
export function validateFacebookPosterUrl(input: string, publicAssetOrigin: string | undefined): string {
  if (!publicAssetOrigin) throw new Error("Project media storage is not configured for Facebook posters.");
  let poster: URL;
  let projectOrigin: URL;
  try {
    poster = new URL(input.trim());
    projectOrigin = new URL(publicAssetOrigin);
  } catch {
    throw new Error("Use an uploaded poster from this project's public media storage.");
  }
  if (poster.protocol !== "https:" || projectOrigin.protocol !== "https:" || poster.origin !== projectOrigin.origin || poster.username || poster.password) {
    throw new Error("Use an uploaded poster from this project's public media storage.");
  }
...
export function getFacebookEmbedAspectRatio(item: { width: number | null; height: number | null; aspect_ratio: string | null }): number {
  if (item.width && item.height) {
    const measured = item.width / item.height;
    if (measured >= 0.35 && measured <= 3) return measured;
  }
  const declared = item.aspect_ratio?.split(":").map(Number);
  if (declared?.length === 2 && declared[0] > 0 && declared[1] > 0) return declared[0] / declared[1];
  return 16 / 9;
}
