-- Allow extended Wren Flight sessions without exhausting the daily reward cap.
...
begin
  ...
  if not found then raise exception 'WREN_FLIGHT_REWARD_POLICY_NOT_FOUND'; end if;
  if v_source_policy.daily_cap >= 1500 then return; end if;
  ...
  insert into public.game_reward_policies (
    ...
    v_new_version_id, v_new_difficulty_id,
    v_source_policy.base_reward, v_source_policy.maximum_reward,
    v_source_policy.repeat_multiplier_bps, 1500, v_source_policy.score_target, now()
  );
