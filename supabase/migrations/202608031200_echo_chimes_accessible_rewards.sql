-- Make Echo Chimes rewards attainable after a three-note melody.
...
begin
  select id into v_game_id from public.games where slug = 'echo-chimes';
  if not found then raise exception 'ECHO_CHIMES_GAME_NOT_FOUND'; end if;
  ...
  if not found then raise exception 'ECHO_CHIMES_PUBLISHED_VERSION_NOT_FOUND'; end if;

  select * into v_source_difficulty
  from public.game_difficulties
  where game_version_id = v_source_version.id and key = 'standard';
  if not found then raise exception 'ECHO_CHIMES_STANDARD_DIFFICULTY_NOT_FOUND'; end if;
  ...
  if found and v_source_policy.score_target <= 3 then return; end if;
  ...
  insert into public.game_difficulties (
    ...
    jsonb_set(coalesce(v_source_difficulty.config, '{}'::jsonb), '{targetSequenceLength}', '3'::jsonb, true),
  ...
  insert into public.game_reward_policies (
    ...
    v_new_version_id, v_new_difficulty_id, 12, 30, 2500, 90, 3, now()
  );
