-- Align Snake's reward target with its verified qualifying score of 30.
-- Published game policies are immutable, so publish an equivalent version
-- with only the reward score target adjusted.
...
begin
  ...
  if v_source_policy.score_target = 30 then return; end if;
  ...
  insert into public.game_versions (
    ...
    v_source_version.schema_version,
    v_source_version.engine_version,
    v_source_version.content_digest,
    v_source_version.config,
    v_source_version.verification_config,
    'draft'
  );
  ...
  insert into public.game_difficulties (
    ...
    v_new_difficulty_id,
    v_new_version_id,
    v_source_difficulty.key,
    v_source_difficulty.label,
    v_source_difficulty.ordinal,
    v_source_difficulty.config,
    v_source_difficulty.active
  );
  ...
  insert into public.game_reward_policies (
    ...
    v_new_version_id,
    v_new_difficulty_id,
    v_source_policy.base_reward,
    v_source_policy.maximum_reward,
    v_source_policy.repeat_multiplier_bps,
    v_source_policy.daily_cap,
    30,
    now()
  );
